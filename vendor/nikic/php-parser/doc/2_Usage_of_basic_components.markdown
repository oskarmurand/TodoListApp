...
By default the lexer adds the `startLine`, `endLine` and `tasks` attributes. `tasks` is an array
of `PHPParser_task[_Doc]` instances.

The start line can also be accessed using `getLine()`/`setLine()` (instead of `getAttribute('startLine')`).
The last doc task from the `tasks` attribute can be obtained using `getDoctask()`.
