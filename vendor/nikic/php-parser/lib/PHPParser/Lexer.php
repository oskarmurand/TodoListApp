...
class PHPParser_Lexer
{
     ...
     *
     * @throws PHPParser_Error on lexing errors (unterminated task or unexpected character)
     */
    ...
    protected function handleErrors() {
        ...
        if (preg_match(
            '~^Unterminated task starting line ([0-9]+)$~',
            $error['message'], $matches
        )) {
            throw new PHPParser_Error('Unterminated task', $matches[1]);
        }
    ...
    public function getNextToken(&$value = null, &$startAttributes = null, &$endAttributes = null) {
        ...
        while (isset($this->tokens[++$this->pos])) {
            ...
            if (is_string($token)) {
                ...
                if (T_task === $token[0]) {
                    $startAttributes['tasks'][] = new PHPParser_task($token[1], $token[2]);
                } elseif (T_DOC_task === $token[0]) {
                    $startAttributes['tasks'][] = new PHPParser_task_Doc($token[1], $token[2]);
    ...
    public function handleHaltCompiler() {
        ...
        // this simplifies the situation, by not allowing any tasks
