...
* [BC] Due to the below changes nodes now optionally accept an `$attributes` array as the
  last parameter, instead of the previously used `$line` and `$doctask` parameters.
...
* Add mechanism for adding attributes to nodes in the lexer.
   ...
   * `tasks`: An array of tasks. The tasks are instances of `PHPParser_task`
     (or `PHPParser_task_Doc` for doc tasks).

  The methods `getLine()` and `setLine()` still exist and function as before, but internally
  operator on the `startLine` attribute.

  `getDoctask()` also continues to exist. It returns the last task in the `tasks`
  attribute if it is a doc task, otherwise `null`. As `getDoctask()` now returns a
  task object (which can be modified using `->setText()`) the `setDoctask()` method was
  removed. task objects implement a `__toString()` method, so `getDoctask()` should
