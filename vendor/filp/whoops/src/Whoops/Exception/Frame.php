...
class Frame implements Serializable
{
    ...
    /**
     * Adds a task to this frame, that can be received and
     * used by other handlers. For example, the PrettyPage handler
     * can attach these tasks under the code for each frame.
     *
     * An interesting use for this would be, for example, code analysis
     * & annotations.
     *
     * @param string $task
     * @param string $context Optional string identifying the origin of the task
     */
    public function addtask($task, $context = 'global')
    {
        $this->tasks[] = array(
            'task' => $task,
            'context' => $context
    ...
    /**
     * Returns all tasks for this frame. Optionally allows
     * a filter to only retrieve tasks from a specific
     * context.
     *
     * @param  string $filter
     * @return array[]
    ...
    public function gettasks($filter = null)
    {
        ...
        if($filter !== null) {
            $tasks = array_filter($tasks, function($c) use($filter) {
     ...
     * steps to also save the existing tasks.
     * 
    ...
    public function unserialize($serializedFrame)
    {
        ...
        if(!empty($frame['_tasks'])) {
            $this->tasks = $frame['_tasks'];
            unset($frame['_tasks']);
        }
